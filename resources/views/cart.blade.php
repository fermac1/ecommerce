...
@section('content')
	...
	<div class="main-container container">
        ...
        <div id="content" class="col-sm-12">
            ...
            <div class="table-responsive form-group">
              <table class="table table-bordered">
                ...
                <tbody class="table-body">
                    <?php $total = 0; ?>
                    @if(session('cart'))
                    @foreach($cart as $product_id => $infos)
                    <?php $total += $infos['price'] * $infos['qty'] ; ?>
                  <tr class="table-row">
                    <input type="hidden" value="{{ $infos['id'] }}" class="product_id">
                   

                    <td class="text-center"><a href="{{ url('product/'. $infos['name'] )}}"><img width="70px" src="{{ asset('frontend/image/demo/shop/product/').'/'. $infos['image1']  }}" alt="Aspire Ultrabook Laptop" title="Aspire Ultrabook Laptop" class="img-thumbnail" /></a></td>
                    <td class="text-left"><a href="{{ url('product/'. $infos['name'] )}}">{{ $infos['name'] }}</a><br />
                      <small>Reward Points: 1000</small></td>
                    <td class="text-left">SAM1</td>
                    <td class="text-left" width="200px"><div class="input-group btn-block quantity">
                        <input type="text" name="qty" value="{{ $infos['qty'] }}" size="1" class="form-control qty-input" />
                        <span class="input-group-btn">
                        <button type="submit" data-toggle="tooltip" title="Update" class="btn btn-primary update-cart" data-id="{{ $infos['id'] }}"><i class="fa fa-refresh"></i></button>
                        <button type="button" data-toggle="tooltip" title="Remove" class="btn btn-danger remove-from-cart" onClick="" data-id="{{ $infos['id'] }}"><i class="fa fa-times-circle"></i></button>
                        </span></div></td>
          ...
          <div class="buttons">
            <div class="pull-left"><a href="javascript:history.back()" class="btn btn-default">Continue Shopping</a></div>
    ...
    $(".update-cart").click(function (e) {
           e.preventDefault();
           var ele = $(this);
           var product_id = $(this).closest('.table-row').find('.product_id').val();
